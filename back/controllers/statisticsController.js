...
const statistics = {
	getOutPatient: async function (req, res, next) {
		try {
			...
			if(position[1]){ // 좌측 Tree에서 기관 선택했을 경우
				queryPos.push(` a1.pos_1 = '${position[1]}' `);
			}
			if(position[2]){ // 좌측 Tree에서 층 선택했을 경우
				queryPos.push(` a1.pos_2 = '${position[2]}' `);
			}
			if(position[3]){ // 좌측 Tree에서 구역 선택했을 경우
				queryPos.push(` a1.pos_3 = '${position[3]}' `);
			}

			if(queryPos.length > 0){
	...
	getLeaves: async function (req, res, next) {
		try {
			...
			",( " + 
				...
				`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
				`  ${queryPosText === "" ? 
					" WHERE b1.sunap_type like '%중간%' " : 
					queryPosText + " AND b1.sunap_type like '%중간%' " } ` + 
			" ) AS `중간건수` " + 
			",( " + 
			...
			",( " + 
				...
				`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
				`  ${queryPosText === "" ? 
					" WHERE b1.sunap_type like '%퇴원%' " : 
					queryPosText + " AND b1.sunap_type like '%퇴원%' " } ` + 
			" ) AS `퇴원건수` " + 
			",( " + 
				"  SELECT COUNT(DISTINCT b1.amount) " + 
				`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
	...
	getWeek: async function (req, res, next) {
		try {
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 2 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 2 " } ` + 
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 3 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 3 " } ` + 
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 4 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 4 " } ` + 
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 5 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 5 " } ` + 
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` + 
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 6 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 6 " } ` + 
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` + 
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 7 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 7 " } ` + 
			...
			`  FROM ${db.device_op_info.name} a1 INNER JOIN ${db.sunap_daily_cnt.name} b1 ON a1.dev_id = b1.dev_id ` +
			`  ${queryPosText === "" ? 
				" WHERE DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 1 " : 
				queryPosText + " AND DAYOFWEEK(DATE_FORMAT(b1.sunap_date , '%Y-%m-%d')) = 1 " } ` + 
	...
	getWaitTime: async function (req, res, next) {
		try {
			...
			// 날짜 조회  // queryPos에 함께 담음
			let to = dayjs(req.query.to);
			if(req.query.term === "weekly"){
				// 당월 조회 or 전월 조회 
				queryPos.push(` DATE_FORMAT(b1.q_date, '%Y-%m') = '${to.format("YYYY-MM")}' `);
			}else{ // "monthly"
				// 년간 조회
				queryPos.push(` DATE_FORMAT(b1.q_date, '%Y') = '${to.format("YYYY")}' `);
			}

			queryPos.push(" b1.workno = 1 ");
